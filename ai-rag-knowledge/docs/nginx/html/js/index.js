/** 与 dev-app 端口一致；页面通过 nginx 同源部署时可改为 '' 或相对路径 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const loadRagOptions = () => {
        ...
        fetch(`${API_BASE}/api/v1/rag/query_rag_tag_list`)
...
function appendMessage(content, isAssistant = false, saveToStorage = true) {
 ...
 function startEventStream(message) {
    ...
    // 示例：${API_BASE}/api/v1/openai/generate_stream?message=Hello&model=gpt-4o-mini
    ...
    // aiModelValue 须为 openai（后端仅有 OpenAiController）；无 ollama 接口时不要选错
    if (ragTag) {
        url = `${API_BASE}/api/v1/${aiModelValue}/generate_stream_rag?message=${encodeURIComponent(message)}&ragTag=${encodeURIComponent(ragTag)}&model=${encodeURIComponent(aiModelModel)}`;
    } else {
        url = `${API_BASE}/api/v1/${aiModelValue}/generate_stream?message=${encodeURIComponent(message)}&model=${encodeURIComponent(aiModelModel)}`;
    }
    ...
    currentEventSource.onmessage = function(event) {
        try {
            ...
            // Spring AI 1.x ChatResponse：增量在 output.text（AssistantMessage），结束见 result.metadata.finishReason
            const generation = data.result ?? data.results?.[0];
            const output = generation?.output;
            const delta = (output?.text ?? output?.content ?? '').toString();
            const finishReason = generation?.metadata?.finishReason
                ?? output?.metadata?.finishReason
